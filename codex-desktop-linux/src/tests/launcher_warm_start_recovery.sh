...
wait_for() {
    ...
    shift
    local attempt
    local max_attempts="${CODEX_TEST_WAIT_ATTEMPTS:-300}"
...
select_pidfd_python() {
    ...
    if [ -n "${CODEX_TEST_PYTHON:-}" ]; then
        candidates+=("$CODEX_TEST_PYTHON")
    fi
    candidates+=(
        /usr/bin/python3
...
PY
    ...
    printf '%s\n' 'launcher warm-start recovery test requires a Python interpreter with pidfd APIs' >&2
    return 1
}

PYTHON_BIN="$(select_pidfd_python)" \
    || fail "could not find a pidfd-capable Python interpreter"
...
process_is_running "$FIRST_ELECTRON_PID" \
...
if process_is_running "$FIRST_ELECTRON_PID"; then
