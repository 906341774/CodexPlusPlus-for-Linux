...
test("current DMG descriptors target only the owning Copilot chunks", () => {
  const settingsChunk =
    "app-initial~app-main~new-thread-panel-page~onboarding-page~projects-index-page~appgen-libra~2gcv58yj-BKm6WXbt.js";
  const modelChunk =
...
test("enabled feature descriptors patch the current app settings chunk", () => {
  ...
  withTempFeatureConfig(["copilot-reasoning-effort"], () => {
    withTempDir((extractedDir) => {
      ...
      const patchedModel = readAsset(extractedDir, currentModelChunk);
      assert.match(patchedModel, /a=\[\.\.\.t\]\.filter/);
      assert.doesNotMatch(patchedModel, /e===`copilot`\?\[/);
