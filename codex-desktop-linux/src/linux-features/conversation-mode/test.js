...
test("current DMG splits dictation, assistant, and composer ownership", () => {
  ...
  const composerAsset =
    "app-initial~app-main~settings-command-menu-section-items~new-thread-panel-page~settings-pag~unq8yzli-B2zGu4cN.js";
  const adjacentComposerAsset =
    "app-initial~app-main~new-thread-panel-page~appgen-library-page~hotkey-window-thread-page~ho~jhj9i1pn-B9TNJTvY.js";

  assert.equal(dictation.pattern.test(currentDictationAsset), true);
  ...
  assert.equal(composer.pattern.test(currentDictationAsset), false);
