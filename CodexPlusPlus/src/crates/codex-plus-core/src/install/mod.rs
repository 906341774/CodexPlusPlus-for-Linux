...
pub fn desktop_entry_file_names() -> (&'static str, &'static str) {
...
pub fn inspect_entrypoints() -> EntryPointState {
    inspect_entrypoints_for_app(None)
}

pub fn inspect_entrypoints_for_app(app_dir: Option<&Path>) -> EntryPointState {
    let root = default_install_root();
    EntryPointState {
        silent_shortcut: ShortcutState::from_candidates(entrypoint_candidates_for_app(
            &root, app_dir, false,
        )),
        management_shortcut: ShortcutState::from_candidates(entrypoint_candidates_for_app(
            &root, app_dir, true,
        )),
    }
}
...
fn entrypoint_candidates(root: &Option<PathBuf>, manager: bool) -> Vec<PathBuf> {
    ...
    if cfg!(windows) {
        ...
        let (silent, manager_entry) = desktop_entry_file_names();
        vec![root.join(if manager { manager_entry } else { silent })]
    }
}

fn entrypoint_candidates_for_app(
    root: &Option<PathBuf>,
    app_dir: Option<&Path>,
    manager: bool,
) -> Vec<PathBuf> {
    ...
    #[cfg(target_os = "linux")]
    {
        candidates.extend(linux_adapter_state_entrypoint_candidates(manager));
        candidates.extend(linux_adapter_entrypoint_candidates(app_dir, manager));
        candidates.extend(linux_desktop_entry_candidates(manager));
    }
    candidates.extend(entrypoint_candidates(root, manager));
...
#[cfg(target_os = "linux")]
fn linux_adapter_entrypoint_candidates(app_dir: Option<&Path>, manager: bool) -> Vec<PathBuf> {
    let Some(app_dir) = app_dir else {
        return Vec::new();
    };
    linux_adapter_install_entrypoint_candidates(&app_dir.join(".codex-plusplus"), manager)
}

#[cfg(target_os = "linux")]
fn linux_adapter_state_entrypoint_candidates(manager: bool) -> Vec<PathBuf> {
    let Some(install_root) = linux_adapter_state_install_root() else {
        return Vec::new();
    };
    linux_adapter_install_entrypoint_candidates(&install_root, manager)
}

#[cfg(target_os = "linux")]
fn linux_adapter_state_install_root() -> Option<PathBuf> {
    let state_path = linux_adapter_state_file()?;
    let state_text = std::fs::read_to_string(state_path).ok()?;
    let state: serde_json::Value = serde_json::from_str(&state_text).ok()?;
    state
        .get("linuxAdapterInstallRoot")
        ...
        .or_else(|| {
            state
                ...
                .and_then(serde_json::Value::as_str)
                .filter(|value| !value.trim().is_empty())
                .map(|app_root| PathBuf::from(app_root).join(".codex-plusplus"))
        })
}

#[cfg(target_os = "linux")]
fn linux_adapter_state_file() -> Option<PathBuf> {
    ...
    directories::BaseDirs::new()
        .map(|dirs| dirs.data_dir().join("codex-plusplus").join("state.json"))
}

#[cfg(target_os = "linux")]
fn linux_desktop_entry_candidates(manager: bool) -> Vec<PathBuf> {
    let (silent, manager_entry) = desktop_entry_file_names();
    let name = if manager { manager_entry } else { silent };
    let Some(base_dirs) = directories::BaseDirs::new() else {
        return Vec::new();
    };
    vec![base_dirs.data_dir().join("applications").join(name)]
}

#[cfg(target_os = "linux")]
fn linux_adapter_install_entrypoint_candidates(install_root: &Path, manager: bool) -> Vec<PathBuf> {
    let install_dir = install_root.join("install");
    if manager {
        vec![install_dir.join(MANAGER_BINARY)]
    } else {
        vec![
            install_dir.join("launch-codex-plus-plus"),
            install_dir.join(SILENT_BINARY),
        ]
    }
}
