...
pub fn normalize_codex_app_path(path: &Path) -> Option<PathBuf> {
    ...
    if cfg!(target_os = "linux") && path.is_dir() && path.join("start.sh").is_file() {
        return Some(path.to_path_buf());
    }
...
pub fn build_codex_executable(app_dir: &Path) -> PathBuf {
    ...
    let linux_launcher = app_dir.join("start.sh");
    if cfg!(target_os = "linux") && linux_launcher.exists() {
...
fn linux_adapter_state_path() -> PathBuf {
    if let Some(value) = std::env::var_os("XDG_DATA_HOME").filter(|value| !value.is_empty()) {
        return PathBuf::from(value).join("codex-plusplus/state.json");
    }
    directories::BaseDirs::new()
        .map(|dirs| {
            dirs.home_dir()
                .join(".local/share/codex-plusplus/state.json")
        })
        .unwrap_or_else(|| PathBuf::from(".local/share/codex-plusplus/state.json"))
}
...
fn normalize_linux_codex_app_root(path: &Path) -> Option<PathBuf> {
    ...
    path.join("start.sh").is_file().then_some(path)
}
