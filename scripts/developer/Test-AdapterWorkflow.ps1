...
Assert-TextContains 'scripts/developer/Installer-and-Manager.ps1' '[switch]$SkipSnippetApply' 'developer installer supports already-adapted source trees'
Assert-TextContains 'scripts/developer/Installer-and-Manager.ps1' 'Skipping snippet replay by request' 'developer installer logs explicit snippet replay skips'
Assert-TextContains 'scripts/developer/Installer-and-Manager.ps1' 'Assert-SourceLinuxAdaptationApplied -SourceRoot $script:SourceRoot' 'developer installer still verifies skipped snippet trees'
Assert-TextContains 'scripts/developer/Installer-and-Manager.ps1' 'filter_killable_unix_restart_processes' 'developer installer verifies the complete Linux restart process-tree implementation'
...
Assert-TextContains 'scripts/developer/Build-CodexDesktopLinux.ps1' '[string]$UpstreamDmgPath' 'Codex Desktop build accepts an explicit upstream DMG path'
Assert-TextContains 'scripts/developer/Build-CodexDesktopLinux.ps1' 'CODEX_DMG_REFRESH_MODE' 'Codex Desktop build can pin cached DMG reuse'
Assert-TextContains 'scripts/developer/Build-CodexDesktopLinux.ps1' 'DMG=$ResolvedUpstreamDmgPath' 'Codex Desktop build passes explicit DMG path through make'
...
Assert-TextContains 'docs/modules/ROOT/pages/maintenance-workflow.adoc' 'dedicated test `HOME`, Xvfb display, and xdotool' 'English maintenance workflow isolates GUI automation in Podman'
Assert-TextContains 'docs/modules/ROOT/pages/maintenance-workflow_zh-CN.adoc' '独立测试 `HOME`、Xvfb display 与 xdotool' 'Chinese maintenance workflow isolates GUI automation in Podman'
Assert-TextContains 'docs/modules/ROOT/pages/maintenance-workflow.adoc' "Never`ndrive the maintainer host's active desktop" 'English maintenance workflow forbids host desktop automation'
Assert-TextContains 'docs/modules/ROOT/pages/maintenance-workflow_zh-CN.adoc' '禁止对维护者宿主机正在使用的活动桌面执行坐标或窗口自动化' 'Chinese maintenance workflow forbids host desktop automation'
...
Assert-TextContains 'docs/modules/ROOT/pages/snippets.adoc' '086-linux-thread-preview-backfill' 'English snippets thread preview backfill patch'
Assert-TextContains 'docs/modules/ROOT/pages/snippets_zh-CN.adoc' '086-linux-thread-preview-backfill' 'Chinese snippets thread preview backfill patch'
Assert-TextContains 'docs/modules/ROOT/pages/snippets.adoc' '087-linux-restart-process-tree' 'English snippets Linux restart process-tree patch'
Assert-TextContains 'docs/modules/ROOT/pages/snippets_zh-CN.adoc' '087-linux-restart-process-tree' 'Chinese snippets Linux restart process-tree patch'
...
Assert-TextContains 'docs/modules/ROOT/attachments/feature-matrix.json' 'provider_sync_backfills_empty_thread_preview_from_first_user_message' 'Feature matrix tracks empty thread preview repair'
Assert-TextContains 'docs/modules/ROOT/attachments/feature-matrix.json' 'unix_restart_cleanup_includes_launcher_descendants_children_first' 'Feature matrix tracks Linux restart process-tree cleanup'
Assert-TextContains 'docs/modules/ROOT/attachments/feature-matrix.json' 'unix_restart_cleanup_includes_orphans_from_the_launchers_desktop_root' 'Feature matrix tracks reparented Linux Desktop orphan cleanup'
Assert-TextContains 'docs/modules/ROOT/attachments/feature-matrix.json' 'unix_restart_cleanup_uses_the_current_manager_root_when_launcher_is_gone' 'Feature matrix tracks orphan cleanup after launcher exit'
Assert-TextContains 'docs/modules/ROOT/attachments/feature-matrix.json' 'without manually deleting app-server.sock or app-server.sock.lock' 'Feature matrix requires shared app-server stale ownership recovery'
...
Assert-TextContains 'docs/modules/ROOT/attachments/patches/compatibility/086-linux-thread-preview-backfill.patch' 'SET preview = first_user_message' 'Thread preview patch backfills visible legacy conversations'
Assert-TextContains 'docs/modules/ROOT/attachments/patches/compatibility/087-linux-restart-process-tree.patch' 'filter_killable_unix_restart_processes' 'Restart patch collects the full Linux launcher descendant tree'
Assert-TextContains 'codex-desktop-linux/src/linux-features/shared-app-server-socket/patch.js' 'authorityGroupMatches' 'Shared app-server feature validates a recorded orphan process group before cleanup'
Assert-TextContains 'codex-desktop-linux/src/linux-features/shared-app-server-socket/patch.js' 'socketDev' 'Shared app-server feature records socket identity for stale recovery'
Assert-TextContains 'codex-desktop-linux/src/linux-features/shared-app-server-socket/test.js' 'reclaims an old legacy lock when no socket exists' 'Shared app-server tests cover legacy empty lock recovery'
Assert-TextContains 'docs/modules/ROOT/pages/maintenance-workflow.adoc' 'Shared app-server restart gate' 'English maintenance workflow fixes the restart lifecycle acceptance gate'
Assert-TextContains 'docs/modules/ROOT/pages/maintenance-workflow_zh-CN.adoc' 'Shared app-server 重启门禁' 'Chinese maintenance workflow fixes the restart lifecycle acceptance gate'
Assert-TextContains 'docs/modules/ROOT/pages/maintenance/2026-07-25-v1.2.41-fix-0004-shared-app-server-restart-recovery.adoc' 'Manual deletion of `app-server.sock*`' 'Restart recovery maintenance note forbids manual cleanup as acceptance'
Assert-TextContains 'docs/modules/ROOT/nav.adoc' '2026-07-25-v1.2.41-fix-0004-shared-app-server-restart-recovery.adoc' 'Restart recovery maintenance note nav entry'
...
Assert-TextContains 'codex-desktop-linux/src/scripts/build-rpm.sh' 'hash="2.$hash"' 'RPM hotfix releases sort above the numeric baseline release'
Assert-TextContains 'codex-desktop-linux/src/scripts/build-rpm.sh' 'write_launcher_stub "$staging_root"' 'RPM builder reuses the shared package launcher contract'
Assert-TextContains 'codex-desktop-linux/src/scripts/lib/package-common.sh' '.codex-plusplus/install/launch-codex-plus-plus' 'System packages prefer the bundled Codex++ launcher'
...
Assert-TextContains 'docs/modules/ROOT/attachments/patches/compatibility/900-regression-tests.patch' 'provider_sync_backfills_empty_thread_preview_from_first_user_message' 'Regression patch covers empty thread preview repair'
Assert-TextContains 'docs/modules/ROOT/attachments/patches/compatibility/900-regression-tests.patch' 'unix_restart_cleanup_includes_launcher_descendants_children_first' 'Regression patch covers Linux restart process-tree cleanup'
Assert-TextContains 'docs/modules/ROOT/attachments/patches/compatibility/900-regression-tests.patch' 'unix_restart_cleanup_includes_orphans_from_the_launchers_desktop_root' 'Regression patch covers reparented Linux Desktop orphan cleanup'
Assert-TextContains 'docs/modules/ROOT/attachments/patches/compatibility/900-regression-tests.patch' 'unix_restart_cleanup_uses_the_current_manager_root_when_launcher_is_gone' 'Regression patch covers orphan cleanup after the launcher exits completely'
...
if (Test-RepoPathExists '.podman-test') {
    foreach ($file in @(
        ...
        '.podman-test/codexpp-fedora43.Containerfile',
        '.podman-test/README.adoc',
        '.podman-test/pixi.toml',
        '.podman-test/run-codexpp-gui-validation.sh',
        '.podman-test/run-codexpp-fedora43-gui-validation.sh',
    ...
    # EN: Fedora 43 is a separate modern build line and must use its native dependency stack.
    # ZH: Fedora 43 是独立的现代构建线，必须使用发行版原生依赖栈。
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'FROM fedora:43' 'Fedora fixture pins Fedora 43'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'webkit2gtk4.1-devel' 'Fedora fixture uses native WebKitGTK 4.1 development files'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'javascriptcoregtk4.1-devel' 'Fedora fixture uses native JavaScriptCoreGTK 4.1 development files'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'libsoup3-devel' 'Fedora fixture uses native libsoup 3 development files'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'nodejs' 'Fedora fixture uses native Node.js'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'rust cargo' 'Fedora fixture uses native Rust and Cargo'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' '7zip' 'Fedora fixture uses native modern 7-Zip'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'ln -s /usr/sbin/7z /usr/local/bin/7zz' 'Fedora fixture exposes native 7-Zip through the workflow 7zz command name'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'ydotool' 'Fedora fixture uses native ydotool'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'xorg-x11-server-Xvfb' 'Fedora fixture installs Xvfb for GUI validation'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'xdotool' 'Fedora fixture installs xdotool for GUI validation'
    Assert-TextContains '.podman-test/codexpp-fedora43.Containerfile' 'google-noto-sans-cjk-fonts' 'Fedora fixture installs CJK fonts'
    Assert-TextNotContains '.podman-test/codexpp-fedora43.Containerfile' 'pixi' 'Fedora fixture must not copy the AlmaLinux Pixi layer'
    Assert-TextNotContains '.podman-test/codexpp-fedora43.Containerfile' 'conda' 'Fedora fixture must not copy the AlmaLinux Conda layer'
    Assert-TextNotContains '.podman-test/codexpp-fedora43.Containerfile' 'webkit2gtk3-devel' 'Fedora fixture must not use the WebKitGTK 4.0 compatibility ABI'
    Assert-TextNotContains '.podman-test/codexpp-fedora43.Containerfile' 'linux-manager-webkit4-compat' 'Fedora fixture must not use the WebKitGTK 4.0 compatibility bridge'
    Assert-TextNotContains '.podman-test/codexpp-fedora43.Containerfile' 'LD_LIBRARY_PATH' 'Fedora fixture must not inject an alternate runtime linker path'
    Assert-TextNotContains '.podman-test/codexpp-fedora43.Containerfile' 'expat.pc' 'Fedora fixture must not carry the AlmaLinux expat pkg-config workaround'
    Assert-TextContains '.podman-test/run-codexpp-fedora43-gui-validation.sh' 'CODEX_HOME="${CODEXPP_TEST_CODEX_HOME' 'Fedora GUI validation isolates Codex home'
    Assert-TextContains '.podman-test/run-codexpp-fedora43-gui-validation.sh' 'CODEX_PLUS_DISABLE_UPDATE_CHECK=1' 'Fedora GUI validation pins the selected baseline'
    Assert-TextContains '.podman-test/run-codexpp-fedora43-gui-validation.sh' 'dbus-run-session' 'Fedora GUI validation uses a fresh native DBus session'
    Assert-TextNotContains '.podman-test/run-codexpp-fedora43-gui-validation.sh' 'pixi' 'Fedora GUI validation must not depend on Pixi'
    Assert-TextNotContains '.podman-test/run-codexpp-fedora43-gui-validation.sh' 'conda' 'Fedora GUI validation must not depend on Conda'
    Assert-TextNotContains '.podman-test/run-codexpp-fedora43-gui-validation.sh' 'LD_LIBRARY_PATH' 'Fedora GUI validation must use the native runtime linker configuration'
    Assert-TextNotContains '.podman-test/run-codexpp-fedora43-gui-validation.sh' 'WEBKIT_DISABLE_COMPOSITING_MODE' 'Fedora GUI validation must not disable WebKit compositing'
    ...
    Assert-TextContains '.podman-test/README.adoc' 'Fedora 43 native packages' 'Podman README documents the independent modern dependency stack'
    Assert-TextContains '.podman-test/README.adoc' 'must not use the AlmaLinux Pixi/Conda compatibility layer' 'Podman README prohibits carrying EL9 workarounds into Fedora'
    Assert-TextContains '.podman-test/README.adoc' 'run-codexpp-fedora43-gui-validation.sh' 'Podman README documents the native Fedora GUI launcher'
